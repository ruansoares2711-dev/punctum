...
export function AccountController() {
  ...
  useEffect(() => {
    if (!user) return;
    CheckoutService.listPurchases(user.id).then(setPurchases).catch(() => {});
  }, [user]);
                  ...
                  <div key={item.id} className="overflow-hidden rounded-lg border border-border bg-background">
                    <div className="aspect-[4/3] bg-muted">
                      {item.photo?.preview_path && (
                        <img
                          src={`${env.supabaseUrl}/storage/v1/object/public/photos-preview/${item.photo.preview_path}`}
