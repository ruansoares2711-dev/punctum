...
function Cart() {
  ...
  const total = items.reduce((s, i) => s + (i.photo?.price_cents ?? 0), 0);

  const checkout = async () => {
    setPaying(true);
    try {
      const { initPoint } = await checkoutFn({ data: { origin: window.location.origin } });
              ...
              <div key={item.id} className="flex items-center gap-4 rounded-xl border border-border bg-card p-3">
                <div className="h-20 w-28 shrink-0 overflow-hidden rounded-lg bg-muted">
                  {item.photo?.preview_path && (
                    <img src={previewUrl(supabaseUrl, item.photo.preview_path)} alt="" className="h-full w-full object-cover" />
                  )}
                </div>
